...
本文描述 `yanxu-platform 0.2.0` 已实现的架构。言台只提供桌面平台原语；按钮、输入框、
...
FFI 已捕获的恐慌可能让标准互斥锁进入中毒状态。0.2.0 在下一次访问时保留内存安全状态、
清除中毒标记并累计恢复次数，使资源析构和事件循环不会因同一故障再次恐慌。该计数通过
调试快照公开，应用可把非零值纳入故障遥测。

## 后端策略

0.2.0 使用一个 Rust 公共后端，由 `winit` 在编译目标上选择 Windows、macOS、Wayland
...
0.2.0 不提供 GPU 合成、浏览器/WebView、原生控件包装、辅助功能树、打印、系统托盘或
