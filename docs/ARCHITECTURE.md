...
本文描述 `yanxu-platform 0.4.0` 已实现的架构。言台只提供桌面平台原语；按钮、输入框、
...
| `native/src/backend.rs` | 36 个原生操作、权限检查、参数校验与稳定错误代码 |
| `native/src/model.rs` | 不含系统句柄的父子资源图、窗口状态、资源与帧指标 |
| `native/src/event.rs` | 事件协议 v1.1、有界批次、高频事件合并与队列水位 |
...
逻辑像素和当前 DPI 比例生成整张 CPU 像素图，再一次呈现到原生表面；内部代次防止旧帧
完成时清除新帧状态。绘制期间不会逐命令跨越 ABI。
...
FFI 已捕获的恐慌可能让标准互斥锁进入中毒状态。0.4.0 在下一次访问时保留内存安全状态、
...
0.4.0 使用一个 Rust 公共后端，由 `winit` 在编译目标上选择 Windows、macOS、Wayland
...
0.4.0 不提供 GPU 合成、浏览器/WebView、原生控件包装、辅助功能树、打印、系统托盘或
