...
本文描述 `yanxu-platform 0.6.0` 已实现的架构。言台只提供桌面平台原语；按钮、输入框、
...
| `native/src/backend.rs` | 39 个原生操作、权限检查、参数校验与稳定错误代码 |
...
FFI 已捕获的恐慌可能让标准互斥锁进入中毒状态。0.6.0 在下一次访问时保留内存安全状态、
...
焦点/动作请求和拒绝。累计计数使用饱和加法，窗口关闭会扣除当前量但保留高水位和累计
量；诊断读取不会清零或改变运行状态。
...
0.6.0 使用一个 Rust 公共后端，由 `winit` 在编译目标上选择 Windows、macOS、Wayland
...
当前不提供 GPU 合成、浏览器/WebView、原生控件包装、系统无障碍桥、打印、系统托盘或
移动平台。透明“图层”当前是可由保存/恢复包围的透明度状态，不是隔离的离屏合成组。
语义树与原生桥分别协商，这些限制不会通过虚构能力字段隐藏。
