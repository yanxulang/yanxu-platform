...
本文描述 `yanxu-platform 1.0.0` 已实现的架构。言台只提供桌面平台原语；按钮、输入框、
...
FFI 已捕获的恐慌可能让标准互斥锁进入中毒状态。1.0.0 在下一次访问时保留内存安全状态、
...
1.0.0 使用一个 Rust 公共后端，由 `winit` 在编译目标上选择 Windows、macOS、Wayland
