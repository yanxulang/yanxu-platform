...
编号、源码提交、归档摘要和逐文件摘要。独立固定 Linux x86-64 作业生成版本化 Release
性能预算报告；另一独立 Linux 作业用精确 Rust 1.92.0 执行格式、全目标测试和严格
Clippy。`assemble` 作业只有在当前提交的六目标重新构建、最低工具链、依赖审计和性能
预算全部成功后才：
...
5. 核对性能报告的版本、平台、四项预算与最终判定；
6. 上传归档、独立摘要、完整清单和性能报告为同一 CI 候选制品。
