...
当前版本为 `0.7.0`，最低要求言序 `1.1.7`。原生后端采用 ABI v2，窗口层使用
`winit`，CPU 表面使用 `softbuffer`，二维栅格化使用 `tiny-skia`，复杂文字使用
`cosmic-text`。依赖版本、许可证与选型理由会固定在 `docs/` 中。

0.7.0 把同一棵有界语义树接入 Windows UIA、macOS NSAccessibility 与 Linux AT-SPI。
窗口首次显示前注册原生桥，树、标题、尺寸和 DPI 变化会同步；系统焦点与动作请求重新
通过当前修订、节点状态、参数和有界事件队列校验。能力查询如实报告当前原生后端，调试
快照公开桥激活、停用、同步、请求与拒绝计数。
